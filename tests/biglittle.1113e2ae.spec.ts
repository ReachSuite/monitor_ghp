...
test("Customer experience test suite for @biglittle", async ({ page }) => {
  ...
  await expectStepDialog(
    page,
    "Here you can see the list of all the potential leaks quantified."
  );
  await expectStepDialog(
    page,
    "You can also see a trend line for the history of leaks."
  );
  await expectStepDialog(
    page,
    "You can sort by the different stages to look at the leaks at a particular stage."
  );
  ...
  await expectStepDialog(
    page,
    "You can assign a leak to a particular GTM leader or a member from your RevOps team."
  );
  await expectDialog(page, "Click here to assign this leak to a user.");
  await page
    .locator("datatable-body-row")
    .filter({ hasText: "Deals Stuck in Pipeline" })
    .getByRole("button")
    .click();
  await expectDialog(page, "Select this user.", true);
  await page
    .getByRole("option", { name: "hamad@biglittle.ai" })
    .locator("div")
    .first()
    .click();
  await expectStepDialog(
    page,
    "The leaks gets assigned to the user and they receive a notification."
  );
  await expectHeading(page, "Book a free POC!🚀");
  await expectText(
    page,
    "To learn more about Revenup, click on the button below for a tailored demo and free POC."
  );
